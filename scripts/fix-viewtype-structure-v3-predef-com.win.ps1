...
function Get-PredefinedRefMap([object]$Connection) {
  ...
  foreach ($code in $codes) {
    ...
    for ($i = 0; $i -lt $count; $i++) {
      ...
      $name = T ($row.Get(2))
      $key = $code + '|' + $name
      if (-not $allByKey.ContainsKey($key)) { $allByKey[$key] = New-Object System.Collections.Generic.List[object] }
      $allByKey[$key].Add($ref)
      $cmpQuery.SetParameter('Ref', $ref)
      $cmp = $cmpQuery.Execute().Unload().Get(0)
      $flags = @([bool]$cmp.Get(0),[bool]$cmp.Get(1),[bool]$cmp.Get(2),[bool]$cmp.Get(3),[bool]$cmp.Get(4),[bool]$cmp.Get(5),[bool]$cmp.Get(6))
      if ($flags -contains $true) {
        $predefByKey[$key] = $ref
      }
    }
  }
  return [pscustomobject]@{ PredefByKey = $predefByKey; AllByKey = $allByKey }
}
...
function Get-CanonicalMap([object]$Connection) {
  $info = Get-PredefinedRefMap -Connection $Connection
  $canonicalByKey = $info.PredefByKey
  $duplicates = New-Object System.Collections.Generic.List[object]
  foreach ($key in $info.AllByKey.Keys) {
    $refs = $info.AllByKey[$key]
    if ($refs.Count -le 1) { continue }
    $canonicalRef = $canonicalByKey[$key]
    $canonicalId = Get-RefIdentity -Connection $Connection -Ref $canonicalRef
    foreach ($ref in $refs) {
      $id = Get-RefIdentity -Connection $Connection -Ref $ref
      if ($id -ne $canonicalId) {
        $parts = $key -split '\|',2
        $duplicates.Add([pscustomobject]@{ Ref = $ref; Code = $parts[0]; Name = $parts[1]; Key = $key })
      }
    }
  }
...
$index.Add('# COM fix view type structure v3 predef')
$index.Add('')
...
foreach ($alias in $Aliases) {
  ...
  $summary.Add('# Fix view type structure v3 predef')
  $summary.Add('')
  ...
  $summary.Add('')
  $summary.Add('- duplicates preserved: yes')

  $summaryPath = Join-Path $OutDir ("{0}_fix_viewtype_structure_v3_predef_{1}.md" -f $stamp, $alias)
...
$indexPath = Join-Path $OutDir ("{0}_fix_viewtype_structure_v3_predef_index.md" -f $stamp)
